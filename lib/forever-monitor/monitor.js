...
var Monitor = exports.Monitor = function (script, options) {
    ...
    if (Array.isArray(script)) {
        ...
        this.scriptArgs = script.slice(1);
    }
    else {
        this.scriptArgs.unshift(script);
    }

    if (this.sourceDir) {
        this.scriptArgs[0] = path.join(this.sourceDir, this.scriptArgs[0]);
    }
...
Monitor.prototype.trySpawn = function () {
    if (/node/.test(this.command) && this.checkFile && !this.childExists) {
        try {
            var stats = fs.statSync(this.scriptArgs[0]);
    ...
    var args = this.commandArgs ? this.commandArgs.concat(this.scriptArgs) : this.scriptArgs;

    if (this.fork) {
        this.spawnWith.stdio = ['pipe', 'pipe', 'pipe', 'ipc'];
        return spawn(this.command, args, this.spawnWith);
    }
